...
var logger = {
  ...
  info: (message) => console.log(logger.format("INFO", message)),
  warn: (message) => console.warn(logger.format("WARN", message)),
  error: (message, err) => {
...
var db = {
  async getConfig(userId) {
    const [rows] = await pool.query("SELECT * FROM configs WHERE user_id = ?", [userId]);
    if (rows.length > 0) {
      return {
        ...rows[0],
  ...
  async updateStatus(userId, status, channelId) {
    if (channelId) {
...
var DiscordManager = class {
  ...
  async getClient(userId, token) {
    let client = this.clients.get(userId);
    const config = await db.getConfig(userId);
    if (!client) {
      client = new Client({ patchVoice: true });
      client.on("ready", () => {
...
async function sendWebhookNotification(config, message, color = 5814783) {
...
async function startServer() {
  ...
  app.post("/api/test-webhook", async (req, res) => {
    try {
      ...
      await sendWebhookNotification(config, "\u{1F514} **Test Notification**: Webhook integration is active and working correctly.", 3447003);
  ...
  app.get("/api/channels/:guildId/:userId", async (req, res) => {
    try {
      ...
      const channels = guild.channels.cache.filter((c) => c.type === "GUILD_VOICE").map((c) => ({
        id: c.id,
        name: c.name
      }));
      res.json(channels);
    ...
    try {
      ...
      connection.on("stateChange", async (oldState, newState) => {
        ...
        if (newState.status === VoiceConnectionStatus.Ready) {
          ...
          setTimeout(() => connection.setSpeaking(false), 1e3);
        }
        ...
        try {
          ...
          if (channel && "guild" in channel) {
            ...
            if (conn) {
              ...
              sendWebhookNotification(config, `\u{1F6AA} **Left** voice channel: \`${channel.name}\` (\`${channel.id}\`)`, 10038562);
            }
          }
        } catch (e) {
